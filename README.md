...
Niramaya is a small installable web app for recording vitals by patient: BP, SpO2, pulse, temperature in Fahrenheit, notes, date, and time. Reading times are saved to Google Sheets with an IST offset for readability. It keeps a local backup in the browser and can sync to one shared Google Sheet through Google Apps Script.
